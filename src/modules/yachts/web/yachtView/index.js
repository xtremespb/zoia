...
export default () => ({
    async handler(req, rep) {
        try {
            ...
            const site = new req.ZoiaSite(req, "yachts", this.mongo.db);
            ...
            const yacht = await this.mongo.db.collection(this.zoiaModulesConfig["yachts"].collectionYachts).findOne(query, {
                projection
            });
            if (!yacht || !yacht[site.language]) {
                rep.callNotFound();
                return rep.code(204);
            }
            const render = await template.stream({
                $global: {
                    ...
                    pageTitle: yacht[site.language].title,
                    extraCSS: yacht[site.language].cssMin,
                    extraJS: yacht[site.language].jsMin,
                    ...await site.getGlobals()
                },
                content: yacht[site.language].contentMin
            });
