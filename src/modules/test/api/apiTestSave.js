...
export default () => ({
    ...
    async handler(req, rep) {
        ...
        const extendedValidation = new req.ExtendedValidation(formData, testEdit.root, testEdit.part, testEdit.files, Object.keys(req.zoiaConfig.languages));
        // Perform validation
        ...
        try {
            ...
            if (await rep.checkDatabaseDuplicates(rep, this.mongo.db, req.zoiaModulesConfig["test"].collectionTest, {
                    dir: data.dir || "",
            ...
            if (id) {
                const dbItem = await this.mongo.db.collection(req.zoiaModulesConfig["test"].collectionTest).findOne({
                    _id: new ObjectId(id)
                });
            ...
            // Update database
            const update = await this.mongo.db.collection(req.zoiaModulesConfig["test"].collectionTest).updateOne(id ? {
                _id: new ObjectId(id)
            } : {
                dir: data.dir || "",
                filename: data.filename || "",
