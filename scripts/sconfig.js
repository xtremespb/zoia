...
const nginxDest = path.resolve(`${__dirname}/../build/configs/nginx_${config.siteOptions.id}.conf`);
const nginxConfigData = fs.readFileSync(nginxSource, "utf8")
    .replace(/{server_name}/gm, config.siteOptions.server)
    .replace(/{site_id}/gm, config.siteOptions.id)
...
const serviceDest = path.resolve(`${__dirname}/../build/configs/${config.siteOptions.id}.service`);
