...
// TODO: add pattern or function to validate value before adding
...
export default class BulmaTagsInput extends Component {
    ...
    _bindEvents() {
        ...
        // Bind event handlers to original input
    ...
    add(items, silently = false) {
        ...
        if (!this._objectItems && (items.filter(item => isString(item)).length !== items.length)) {
            throw new Error("Item must be a string or an array of strings");
        }
        ...
        if (this._objectItems && (items.filter(item => isObject(item)).length !== items.length)) {
            throw new Error("Item must be an object or an array of objects");
        }
    ...
    indexOf(item) {
        ...
        if (this._objectItems) {
            ...
            return this.items.map((e) => e.value).indexOf(item.value);
        }
    ...
    removeAtIndex(index, clearSelection = true) {
        if (this.options.removable && !Number.isNaN(index) && index >= 0 && index < this.items.length) {
    ...
    _onInputKeyPress(e) {
        ...
        if (this._manualInputAllowed && (value.includes(this.options.delimiter) || key === 13)) {
            ...
            values.forEach(val => {
                ...
                if ((val = val.replace(this.options.delimiter, "")) !== "") {
