...
export default async (fastify, data, socket) => {
    try {
        if (data && data.token && data.id && fastify.redis) {
            ...
            const lockData = await fastify.redis.get(`${fastify.zoiaConfig.id}_pages_lock_${data.id}`);
            if (!lockData) {
                await fastify.redis.set(`${fastify.zoiaConfig.id}_pages_lock_${data.id}`, user._id);
                socket.lockData = {
                    module: "pages",
                    id: data.id
                };
                ...
                if (lockUser) {
                    socket.emit("pages.alreadyLocked", lockUser.displayName || lockUser.username);
                }
            } else if (lockData && lockData === String(user._id)) {
                socket.lockData = {
                    module: "pages",
